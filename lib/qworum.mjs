 ...
 * ```javascript
 * await Qworum.setData('year', Json(2024));
 * ```
 * 
...
class Qworum {
  ...
  static async checkAvailability() {
    // make sure UI cannot go out of sync with the app state
    history.back();
   ...
   * @example Evaluate a Qworum script:
   * ```javascript
   * await Qworum.eval(
   *   Script(
   *     Goto('next-phase/')
  ...
  static async eval(script) {
    ...
    // make sure UI cannot go out of sync the app state
    history.back();
   ...
   * @example Set the value of a data container:
   * ```javascript
   * try{
   *   await Qworum.setData('year', Json(2022));
   *   console.info('The write operation was successful.');
   * }catch(error){
   *   console.error('The write operation was not successful.');
   * }     
   * ```
   * @see {@link https://qworum.net/en/specification-v1.0/#data|Data container definition in the Qworum specification}
   */
   ...
   * @example Read the value of a data container:
   * ```javascript
   * try{
