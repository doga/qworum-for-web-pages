...
class Qworum {
  ...
  /**
   * 
   * @param {string} userObjectName if the persona isn't set yet for the session, then call qworum://user/set-persona for this Qworum object. Default value is 'user'.
   * @returns {Promise<(Persona|null)>}
   */
  static async getPersona(userObjectName){
    Qworum._log(`[getPersona] `);
    try {
      // ►► call the browser to read the persona for the current Qworum session
      const
      ...
      personas  = Persona.readFrom(personaSd.value);
        ...
        if (userObjectName) {
          ...
          this.eval(
            Script.build(
              Sequence.build(
                Try.build(
                  Call.build(userObjectName ?? 'user', 'qworum://*user/set-persona-for-session'), // which Qworum object name ?
                  {
                    // if the end-user did not select a persona ...
                    catch: 'cancelled', 
      ...
      if (personas.length !== 1) {
        throw new Error('error getting session persona');
      }

      const persona = personas[0];
      persona.personaId = null; // Qworum services don't need to know the persona id.
