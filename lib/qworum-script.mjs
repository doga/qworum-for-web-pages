...
class SemanticData extends GenericData { // BUG non-absolute IRIs must be resolved, based on the URL of the webpage/endpoint that generated the dataset.
  static tag = 'semantic';
  _value; // N3.Store
  _prefixes;
  ...
  toXmlElement(namespaceStack) {
    ...
    children       = [new XmlText(this.toRawString())]  ,
  ...
  static fromIndexedDb(encodedData) {
    if (!(
      encodedData && typeof encodedData === 'object' && !Array.isArray(encodedData) && 
      encodedData.type === `${SemanticData.namespace} ${SemanticData.tag}` && 
      typeof encodedData.value === 'object' && typeof encodedData.value.text === 'string'
    )) throw new Error('wrong IndexedDB object');
    const res = new SemanticData();
    res.readFromText(encodedData.value.text);
    return res;
  }
  toIndexedDb() {
    return {
      ...
      value: {
        text: this.toRawString()
      }
    };
  }
}
SemanticData.prototype.getTag = function() {
