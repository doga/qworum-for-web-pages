...
class Model {
  ...
  constructor(id, semanticData) {
    ...
    if (semanticData) {
      ...
      if (!semanticData.every(s => s && (typeof s.getTag === 'function') && s.getTag() === 'semantic'))throw new TypeError('not a semantic data');
    }
  ...
  get idsArray() { 
    const res = [];
    for (const [id,_] of this.ids.entries()) res.push(id);
    return res;
  }
